...
const data = [
  { rank: 1, name: '이오스', ticker: 'EOS', rsi: 22.5, status: '과매도', price: 800, isPositive: false },
  { rank: 2, name: '트론', ticker: 'TRX', rsi: 24.1, status: '과매도', price: 160, isPositive: false },
  { rank: 3, name: '스텔라루멘', ticker: 'XLM', rsi: 25.8, status: '과매도', price: 120, isPositive: true },
];

export default function RsiBottom5() {
  return (
            ...
            <TooltipProvider>
                <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs p-4 bg-zinc-900 border-zinc-700 text-zinc-200">
                         <h4 className="font-bold mb-2 text-white">RSI 과매도 (30 이하)</h4>
                        <p className="text-sm mb-2">단기간에 매도세가 몰려 가격이 지나치게 하락한 상태입니다. 통계적으로 저점일 확률이 높아, 곧 기술적 반등이나 상승 추세로 전환될 가능성이 있습니다.</p>
            ...
            {data.map((item) => (
              <TableRow key={item.ticker} className="border-zinc-700">
                ...
                <TableCell className="text-right px-2">
                  <Badge variant="bullish" className="whitespace-nowrap text-xs">{item.status}</Badge>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
