...
const PositionBar = ({ long, short }: { long: number, short: number }) => {
    ...
    return (
        <div className="w-full h-5 flex overflow-hidden rounded-sm relative">
            {/* Background (Short - Red) */}
            <div className="absolute inset-0 bg-[#FF4D4D]/90" />
            {/* Foreground (Long - Green) */}
            <div className="absolute left-0 top-0 bottom-0 bg-[#00C076]/90 transition-all duration-500 border-r border-white/20" style={{ width: `${longPercent}%` }} />
            
             <div className="absolute inset-0 flex items-center justify-between px-2 text-xs font-bold text-white z-10 drop-shadow-sm">
                <span>{longPercent.toFixed(1)}%</span>
            ...
            <CardHeader className="pb-3 pt-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <CardTitle className="font-headline text-base">온체인 고래 롱/숏 비율</CardTitle>
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger>
                                    <HelpCircle className="h-4 w-4 text-muted-foreground" />
                                </TooltipTrigger>
                                <TooltipContent className="max-w-[250px] break-keep text-xs">
                                    <p>온체인 선물 시장 자산기준 500억원 이상 고래 상위 포지션 롱/숏비율입니다.</p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    </div>
